...
color = ['-b' '-r' '-y' '-g']
label = ['0.01' '0.03' '0.1' '0.3']
for i = 1:4
    theta = zeros(3, 1);
    [theta, J_history] = gradientDescentMulti(X, y, theta, alpha(i), num_iters);
    hold on;
    plot(1:numel(J_history), J_history, color(i), 'LineWidth', 2,'DisplayName',['learning rate + ' num2str(alpha(i))]);  
end
legend(gca,'show')
...
area = 1650;
area = (area-mu(1))/sigma(1);

rms = 3;
rms = (rms-mu(2))/sigma(2);
x = [1;area;rms];
